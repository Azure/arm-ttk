...
foreach ($pwb in $passwordBoxes) { # Loop over each password box
    ...
    if (-not $theOutput) { # If we couldn't find the output,
        Write-TtkMessage -MarketplaceWarning $MarketplaceWarning "Could not find $($pwb.Name) in outputs" -TargetObject $pwb # write and error
    ...
    if (-not $MainTemplateParam) {
        Write-TtkMessage -MarketplaceWarning $MarketplaceWarning "Password box $($pwb.Name) is missing from main template parameters "-TargetObject $pwb
        continue
    }
    ...
    if (($MainTemplateParam.type -ne 'SecureString') -and ($MainTemplateParam.type -ne 'SecureObject')) {
        # write an error.
        Write-TtkMessage -MarketplaceWarning $MarketplaceWarning "PasswordBox controls must use secureString or secureObject parameter types.  The Main template parameter '$($pwb.Name)' is a '$($MainTemplateParam.type)'" -TargetObject @($pwb, $MainTemplateParam)
    }
}