...
if (-not $CreateUIDefinitionObject.parameters.outputs) { # If CreateUIDefinition has no outputs
    Write-TtkMessage -MarketplaceWarning $MarketplaceWarning "Outputs is missing from CreateUIDefinition" -ErrorId CreateUIDefinition.Missing.Outputs # error
    return # and return.
}
if (-not $CreateUIDefinitionObject.parameters.outputs.location) { # If outputs does not have a location    
    Write-TtkMessage -MarketplaceWarning $MarketplaceWarning "Location is missing from outputs" -ErrorId CreateUIDefinition.Missing.Outputs.Location # error
    return # and return.
}
...
if ("$($CreateUIDefinitionObject.parameters.outputs.location)".Trim() -ne '[location()]') { # Last, make sure that the location's trimmed value is [location()].
    Write-TtkMessage -MarketplaceWarning $MarketplaceWarning "CreateUIDefinition.outputs.location must be [location()]" -ErrorId CreateUIDefinition.Incorrect.Outputs.Location -TargetObject $CreateUIDefinitionObject.parameters.outputs.location
}