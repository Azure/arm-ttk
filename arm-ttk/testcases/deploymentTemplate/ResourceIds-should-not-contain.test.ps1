...
$items = @([Regex]::Matches($TemplateText, "\s{0,}\[.*resourceId\s{0,}\(\s{0,}resourceGroup\(")) +            # resourceId(resourceGroup(
         ...
         @([Regex]::Matches($TemplateText, "\s{0,}\[.*tenantResourceId\s{0,}\(\s{0,}concat\s{0,}\(")) +       # tenantResourceId(concat(
         @([Regex]::Matches($TemplateText, "\s{0,}\[.*extensionResourceId\s{0,}\(\s{0,}concat\s{0,}\(")) +    # extensionResourceId(concat(
         @([Regex]::Matches($TemplateText, "\s{0,}\[.*subscriptionResourceId\s{0,}\(\s{0,}subscription\(")) + # subscriptionResourceId(subscription(
         @([Regex]::Matches($TemplateText, "\s{0,}\[.*subscriptionResourceId\s{0,}\(\s{0,}concat\s{0,}\(")) + # subscriptionResourceId(concat(
         @([Regex]::Matches($TemplateText, "\s{0,}\[.*reference\s{0,}\(\s{0,}concat\s{0,}\(")) +              # reference(concat(
         @([Regex]::Matches($TemplateText, "\s{0,}\[.*list\w{1,}\s{0,}\(\s{0,}concat\s{0,}\("))               # list*(concat(
