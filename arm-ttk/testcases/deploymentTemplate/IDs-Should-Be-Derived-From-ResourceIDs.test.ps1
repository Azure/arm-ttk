...
foreach ($id in $ids) {
    ...
    if (-not $myId) {
        Write-TtkMessage -MarketplaceWarning $MarketplaceWarning "Blank ID Property found: $($id | Out-String)" -TargetObject $id -ErrorId ResourceId.Is.Missing
        continue
    }
    ...
    if ($expandedId -is [string] -and $expandedId -notmatch $exprMatch) {
        Write-TtkMessage "Property: `"$($id.propertyName)`" must use one of the following expressions for an resourceId property:
            $($allowedExpressions -join ',')" ` -MarketplaceWarning $MarketplaceWarning -TargetObject $id -ErrorId ResourceId.Should.Contain.Proper.Expression
