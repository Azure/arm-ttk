...
foreach ($id in $ids) {
    ...
    if (-not $myId) {
        Write-Error "Blank ID Property found: $($id | Out-String)" -TargetObject (Set-RuleID -RuleIDStart $RULE_ID_START -RuleNumber 1 -TargetObject $id) -ErrorId ResourceId.Is.Missing
        continue
    }
    ...
    if ($expandedId -is [string] -and $expandedId -notmatch $exprMatch) {
        Write-Error "Property: `"$($id.propertyName)`" must use one of the following expressions for an resourceId property:
            ...
            -TargetObject (Set-RuleID -RuleIDStart $RULE_ID_START -RuleNumber 2 -TargetObject $id) -ErrorId ResourceId.Should.Contain.Proper.Expression
    }
}
