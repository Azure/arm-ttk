...
if ($artifactslocationParameter -ne $null) {
    if ($artifactslocationParameter.type -ne "string" -and $artifactslocationParameter.type -ne "secureString" ) {
        Write-Error "The _artifactsLocation in `"$TemplateFileName`" parameter must be a 'string' type in the parameter declaration `"$($artifactslocationParameter.type)`"" -ErrorId ArtifactsLocation.Parameter.TypeMisMatch -TargetObject (Set-RuleID -RuleID $RULE_ID -TargetObject $artifactslocationParameter)
    }
    # is the sasToken present
    if ($artifactslocationSasTokenParameter -eq $null) {
        Write-Error "Template `"$TemplateFileName`" is missing _artifactsLocationSasToken parameter" -ErrorId ArtifactsLocation.Parameter.sasToken.Missing -TargetObject Set-RuleID -RuleID $RULE_ID -TargetObject $artifactslocationParameter
    }
    elseif ($artifactslocationSasTokenParameter.type -ne "secureString") {
        Write-Error "The _artifactsLocationSasToken in `"$TemplateFileName`" parameter must be of type 'secureString'." -ErrorId ArtifactsLocation.Parameter.sasToken.TypeMisMatch -TargetObject (Set-RuleID -RuleID $RULE_ID -TargetObject $artifactslocationParameter)        
    }
    ...
    if ($IsMainTemplate) {
        ...
        if (-not $artifactslocationParameter.defaultValue) {
            ...
            Write-Error "The _artifactsLocation parameter in `"$TemplateFileName`" must have a defaultValue in the main template" -ErrorId ArtifactsLocation.Parameter.DefaultValue.Missing -TargetObject (Set-RuleID -RuleID $RULE_ID -TargetObject $artifactslocationParameter)
        }
        else {
            ...
            if (!($artifactslocationParameter.defaultValue -eq $QuickStartPath -or 
                  $artifactsLocationParameter.defaultValue -like "*deployment().properties.templateLink*")) {
                    Write-Error "The _artifactsLocation in `"$TemplateFileName`" has an incorrect defaultValue, found: $($artifactsLocationParameter.defaultValue)" -ErrorId ArtifactsLocation.Parameter.DefaultValue.Incorrect -TargetObject (Set-RuleID -RuleID $RULE_ID -TargetObject $artifactslocationParameter)
        ...
        if ( !($artifactslocationSasTokenParameter.defaultValue) -and $artifactslocationSasTokenParameter.defaultValue -ne "") {
            Write-Error "The _artifactsLocationSasToken in `"$TemplateFileName`" has an incorrect defaultValue, must be an empty string" -ErrorId ArtifactsLocation.Parameter.sasToken.DefaultValue.Incorrect -TargetObject (Set-RuleID -RuleID $RULE_ID -TargetObject $artifactslocationParameter)
        }
    }
    else {
        ...
        if ($artifactslocationParameter.defaultValue -or 
            ...
            Write-Error "The _artifactsLocation and _artifactsLocationsSasToke parameters in `"$TemplateFileName`" must not have a defaulValue in a nested template." -ErrorId ArtifactsLocation.Parameter.DefaultValue.NotEmpty -TargetObject (Set-RuleID -RuleID $RULE_ID)
        }    
    } 
