...
foreach ($dr in $deploymentResources) {
    ...
    if ($dbg) {
        if ($dbg -is [string] -and  $dbg -ne 'None') {
            Write-Error "Deployment Resources must have no DebugSettings property, or must set it to 'None'" -TargetObject (Set-RuleID -RuleID $RULE_ID -TargetObject $dr) -ErrorId "Deployment.Resource.Has.DebugSetting"
        }
        elseif (
            ...
            Write-Error "Deployment Resources must have no DebugSettings.logDetail property, or must set it to 'None'" -TargetObject (Set-RuleID -RuleID $RULE_ID -TargetObject $dr) -ErrorId "Deployment.Resource.Has.LogDetail"
        }
    }
}