...
foreach ($found in $foundObjects) { # Walk over each found object
    foreach ($prop in $found.psobject.properties) { # then walk thru each property
        ...
        $foundBadFunction = $prop.Value | ?<ARM_Template_Function> -FunctionName ($FunctionNotAllowedInUri -join '|') -Extract
        if ($foundBadFunction) {
            Write-Error "Function'$($foundBadFunction.FunctionName)' found within '$($prop.Name)" -TargetObject $found -ErrorId "URI.Improperly.Constructed"
        }
    }
}
