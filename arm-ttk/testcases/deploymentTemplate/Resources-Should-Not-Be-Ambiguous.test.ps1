...
:nextResourceId foreach ($rid in $resourceIdFunctions) {
                ...
                if ($proxyResourceTypes -notcontains $foundResourceType) {
                    if ($n -eq 0) {
                        ...
                        if ((-not $foundResource) -or ($foundResource | Where-Object Condition)) { 
                            Write-Error "At least one parameter must preceed the resource type in: $rid" -TargetObject (Set-RuleID -RuleIDStart $RULE_ID_START -RuleNumber 1 -TargetObject $rid) -ErrorId 'ResourceID.Missing.Name'
    ...
    if ($foundResourceType) {
        ...
        if ($foundResourceType.EndsWith('/')) {    
            Write-Error "ResourceType has a trailing slash '$foundResourceType'" -TargetObject (Set-RuleID -RuleIDStart $RULE_ID_START -RuleNumber 2 -TargetObject $rid) -ErrorId 'ResourceID.Trailing.Slash'
        }        
        ...
        foreach ($resource in $resourcesOfType) {
            ...
            if ($foundParametersInResource.Count -gt 1) {
                ...
                if ($foundParametersInResource.Count -lt $additionalParameters.Count) {
                    ...
                    Write-Error "Resource referencing $rid does not contain all segments of it's resource name" -TargetObject (Set-RuleID -RuleIDStart $RULE_ID_START -RuleNumber 3 -TargetObject $rid) -ErrorId 'ResourceID.Missing.Name'
                }
                else {
        ...
        foreach ($resource in $resourcesOfSubType) {
            ...
            if ($foundParametersInResource.Count -gt 1) {
                ...
                if ($foundParametersInResource.Count -lt $additionalParameters.Count) {
                    # If we didn't have enough, we may want to write an error.
                    Write-Error "Resource referencing $rid does not contain all segments of it's resource name" -TargetObject (Set-RuleID -RuleIDStart $RULE_ID_START -RuleNumber 4 -TargetObject $rid) -ErrorId 'ResourceID.Missing.Name'
                }
                else {
                    # but if we did, we can finally feel ok about this resourceID.
                    continue nextResourceID
