...
foreach ($parameterInfo in $templateObject.parameters.psobject.properties) {
    ...
    if ($parameter.psobject.properties.item('maxValue')) {
        if ($parameter.maxValue -isnot [long] -and $parameter.maxValue -isnot [int]) {  # PS Core is interpreting the int/longs as long
            Write-TtkMessage -MarketplaceWarning $MarketplaceWarning "$($ParameterName) maxValue is not an [int] or [long] (it's a [$($parameter.maxValue.GetType())])" `
    ...
    if ($parameter.psobject.properties.item('minValue')) {
        if ($parameter.minValue -isnot [long] -and $parameter.minValue -isnot [int]) {
            Write-TtkMessage -MarketplaceWarning $MarketplaceWarning "$($ParameterName) minValue is not an [int] or [long] (it's a [$($parameter.minValue.GetType())])" `
    ...
    if ($max -eq $null -and $min -ne $null){
        Write-TtkMessage -MarketplaceWarning $MarketplaceWarning "$ParameterName missing max value" -ErrorId Parameter.Missing.Max -TargetObject $parameter           
    }

    if ($max -ne $null -and $min -eq $null){
        Write-TtkMessage -MarketplaceWarning $MarketplaceWarning "$ParameterName missing min value" -ErrorId Parameter.Missing.Min -TargetObject $parameter           
    }
}