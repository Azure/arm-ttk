...
foreach ($av in $allApiVersions) {
    ...
    if ($av.ApiVersion -isnot [string]) {
        ...
        # write an error
        Write-Error "Api Versions must be strings" -TargetObject (Set-RuleID -RuleID $RULE_ID -TargetObject $av) -ErrorId ApiVersion.Not.String
    ...
    if (-not $hasDate) {
        ...
        Write-Error "Api versions must be a fixed date. $FullResourceType is not." -TargetObject (Set-RuleID -RuleID $RULE_ID -TargetObject $av) -ErrorId ApiVersion.Not.Date
    ...
    if ($av.ApiVersion -like '*-*-*-*') {
        ...
        if ($howOutOfDate -gt 0 -and $moreRecent -notlike '*-*-*-*') {
            Write-Error "$FullResourceType uses a preview version ( $($av.apiVersion) ) and there are more recent versions available." -TargetObject (Set-RuleID -RuleID $RULE_ID -TargetObject $av) -ErrorId ApiVersion.Preview.Not.Recent
        ...
        if ($howOutOfDate -eq 0 -and $validApiVersions.Count -gt 1){
            ...
            if ($nextApiVersion -eq $truncatedApiVersion){
                Write-Error "$FullResourceType uses a preview version ( $($av.apiVersion) ) and there is a non-preview version for that apiVersion available." -TargetObject (Set-RuleID -RuleID $RULE_ID -TargetObject $av) -ErrorId ApiVersion.Preview.Version.Has.NonPreview
                Write-Output "Valid Api Versions:`n$recentApiVersions"
            }
        }
    }
    ...
    if (($timeSinceApi.TotalDays -gt $NumberOfDays) -and ($howOutOfDate -gt 0)) {
        ...
        if (-not $nonPreviewVersionInUse) {
            ...
            Write-Error "Api versions must be the latest or under $($NumberOfDays / 365) years old ($NumberOfDays days) - API version $($av.ApiVersion) of $FullResourceType is $([Math]::Floor($timeSinceApi.TotalDays)) days old" -ErrorId ApiVersion.OutOfDate -TargetObject (Set-RuleID -RuleID $RULE_ID)
            Write-Output "Valid Api Versions:`n$recentApiVersions"
        }
    }
