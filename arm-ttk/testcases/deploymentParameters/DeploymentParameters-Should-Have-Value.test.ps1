...
foreach ($p in $ParameterObject.parameters.psobject.properties) {
  ...
  if ( -not ($p.Value.PsObject.properties["value"] -or $p.Value.PSObject.properties["reference"])) {
    Write-TtkMessage -MarketplaceWarning $MarketplaceWarning -ErrorId Parameters.Parameter.Missing.Value -Message "'$($p.Name)' must have a property 'value' or 'reference'    
  ...
  if( $p.Value.PsObject.Properties["reference"]){
    ...
    if(-not $kvRef.PsObject.Properties["keyVault"]){
      Write-TtkMessage -MarketplaceWarning $MarketplaceWarning -ErrorId Parameters.Parameter.Missing.Value -Message "'$($p.Name)'.reference is missing have a property 'keyVault'" -TargetObject $p.Value.reference
      continue
    }
    else {
      $kv = $kvRef.keyVault;
      if(-not $kv.PsObject.Properties["id"]){
        Write-TtkMessage -MarketplaceWarning $MarketplaceWarning -ErrorId Parameters.Parameter.Missing.KeyVault.Id -Message "'$($p.Name)'.reference is missing have a property 'keyVault'" -TargetObject $p.Value.reference.keyVault
        continue
      }
      ...
      elseif($kv.id -inotmatch '/subscriptions/([0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12})/resourceGroups/([\w\d_-]{1,64})/providers/Microsoft.KeyVault/vaults/([\w\d-]{3,24})') {
        Write-TtkMessage -MarketplaceWarning $MarketplaceWarning -ErrorId Parameters.Parameter.Bad.KeyVault.Id -TargetObject $p.Value.reference.keyVault.id -Message "'$($p.Name)'.reference.keyVault.id contains improper value, `r`n Should be like : /subscriptions/<subscription id>/resourceGroups/<resource group name>/providers/Microsoft.KeyVault/vaults/<key vault name>"
        continue
      }
    }
    if(-not $kvRef.PsObject.Properties["secretName"]){
      Write-TtkMessage -MarketplaceWarning $MarketplaceWarning -ErrorId Parameters.Parameter.Missing.SecretName -TargetObject $p.Value.reference -Message "'$($p.Name)'.reference is missing have a property 'secretName'"
      continue
    }
    
    if($kvRef.secretName -notmatch '^([A-Za-z0-9\-]{1,127})$'){
      Write-TtkMessage -MarketplaceWarning $MarketplaceWarning -ErrorId Parameters.Parameter.Bad.SecretName -TargetObject $kvRef.secretName -Message "'$($p.Name)'.reference.secretName should only contain alphanumeric caracters or dashes and be between 1 and 127 in length, `r`n Found: $($kvRef.secretName)"
      continue
    }
  }
}
